...
public class GooeyHandler implements IGuiHandler {
    ...
    public Object getClientGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
        if (ID == 0) return new GuiCrafting(player.inventory, world, x, y, z);
        else if (ID == 1) return new GUIExtremeCrafting(
                player.inventory,
                world,
                x,
                y,
    ...
    public Object getServerGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
        if (ID == 0) return new ContainerCustomWorkbench(player.inventory, world, x, y, z);
        else if (ID == 1) return new ContainerExtremeCrafting(
                player.inventory,
                world,
                x,
                y,
