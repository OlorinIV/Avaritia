...
public class TileEntityCompressor extends TileLudicrous implements ISidedInventory {
    ...
    public void updateEntity() {
        ...
        if (input != null && (processing == null || progress < target)) {
            if (CompressorManager.getOutput(input) != null
                    && (output == null || CompressorManager.getOutput(input).isItemEqual(output))) {
                if (processing == null) {
        ...
        if (progress >= target && processing != null
                && (output == null || (output.isItemEqual(processing)
                        && output.stackSize + processing.stackSize <= output.getMaxStackSize()))) {
            if (output == null) output = processing.copy();
            else if (output.isItemEqual(processing)) output.stackSize += processing.stackSize;
    ...
    public void readCustomNBT(NBTTagCompound tag) {
        ...
        if (processing != null) {
            ...
            if (target != 0) {
                ...
                if (tag.hasKey("Ingredient")) this.ingredient = tag.getString("Ingredient");
    ...
    public void writeCustomNBT(NBTTagCompound tag) {
        ...
        } else tag.removeTag("Input");
        if (processing != null) {
            NBTTagCompound produce = new NBTTagCompound();
            processing.writeToNBT(produce);
            tag.setTag("Processing", produce);
            tag.setInteger("Progress", this.progress);
            if (ingredient != null) tag.setString("Ingredient", this.ingredient);
            else tag.removeTag("Ingredient");
        } else {
    ...
    public boolean isItemValidForSlot(int slot, ItemStack stack) {
        ...
        if (slot == 0) {
            ...
            if (CompressorManager.getOutput(stack) == null) return false;
            if (CompressorManager.getOutput(stack).isItemEqual(processing)) return true;
        }
        return false;
    }
